...
<h1 class="text-2xl lg:text-4xl flex-none font-bold text-center">{$_('setup.subscribe.title')}</h1>
<h3 class="text-base font-medium text-center mt-3 max-w-[600px] mx-auto">
  {$_('setup.subscribe.subtitle')}
</h3>
...
>
  {#if !subscribed}
    ...
    >
      {$_('setup.subscribe.description')}
    </h3>
    <div class="max-w-[280px] mx-auto">
      <FormContainer
        on:submit={subscribe}
        submit_label={$_('setup.subscribe.submitButton')}
      ...
      >
        <FormElement
          id="email"
          inputType="input"
          label={$_('setup.subscribe.emailLabel')}
          bind:value={email}
        />
      </FormContainer>
    </div>
  {:else}
    ...
    >
      ...
      {$_('setup.subscribe.successMessage')}
    </div>
    <p class="text-center">{$_('setup.subscribe.thankYouMessage')}</p>
  {/if}
</div>
...
<div class="flex-none flex flex-col place-content-center md:flex-row gap-4">
  <a href="/setup/connect_providers">
    <button class="btn {subscribed ? 'btn-primary' : ''} w-full min-w-[130px]">
      {$_('setup.subscribe.continueButton')} {subscribed ? "" : $_('setup.subscribe.withoutSubscribing')}
    </button></a
  >
</div>
